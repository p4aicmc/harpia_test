...
def test_client(hardware, map, mission, mission_file):
    ...
    while client.get_state() in [GoalStatus.PENDING, GoalStatus.ACTIVE]:
        ...
        if(i):
            # print(client.cancel_goal())
            # print(GoalStatus.ABORTED) # 4
            # print(GoalStatus.RECALLED) # 8
            # print(GoalStatus.PREEMPTED)# 2
            # print('waiting to cancell')
            # colocar em outro topico ao inves de colocar no actionlib??
            # while client.get_state() not in [GoalStatus.ABORTED, GoalStatus.RECALLED, GoalStatus.PREEMPTED]:
            while feedback == 0:
                client.cancel_goal()
                # print(feedback)
                rate.sleep()
