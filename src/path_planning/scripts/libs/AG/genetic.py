...
from libs.AG.utils import pairwise, point_in_polygon, segment_in_polygon, euclidean_distance

# from genetic.utils import _distance_wp_area, _prob_collision

from libs.AG.model import CartesianPoint  # , Version
...
class Genetic:
    ...
    def run(self, max_exec_time=None, verbose=False, info=False, debug=False):
        ...
        while not self.stop_criteria():
            ...
            while not self.converge() and not self.stop_criteria():
                self.flag_newborn = 0
                self.generation += 1
    ...
    def _fitness(self, subject, mapa):
        ...
        elif self.planning_mode == 'chance_constraint':
            return self._fitness_chance_constraint(subject, mapa)
